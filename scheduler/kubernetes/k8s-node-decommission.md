...
If you plan to remove Portworx from a node, applications running on that node using Portworx need to be migrated. If Portworx is not running, the application containers will fail to start since Portworx isn't available locally to mount and attach it's volume(s).
...
If you need to continue using the Kubernetes node without Portworx, you will need to ensure your application pods using Porworx volumes don't get scheduled on this node. 
...
* Now add a node anti-affinity section in your application pod specs to run only on nodes that do _not_ have this label. For e.g
```yaml
