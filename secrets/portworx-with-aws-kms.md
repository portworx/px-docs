...
- Use `secretType=aws` to specify the secret type as aws
- Use `cluster_secret_key=<key>` to set the cluster-wide secret ID. This kms data key associated with the secretID will be used as a passphrase for encrypting volumes.
- Use `env=KEY1=VALUE1,KEY2=VALUE2` to set [Portworx aws environment variables](#aws-kms-env) to identify AWS endpoint.
